...
describe('useDocumentSpecificationPage', () => {
  ...
  it('allows an incomplete model to be saved as unavailable', () => {
    ...
    act(() => {
      result.current.form.setValue('name', 'Modelo indisponível', { shouldDirty: true })
      result.current.form.setValue('status', 'unavailable', { shouldDirty: true })
    })
  ...
  it('confirms before changing tabs when an edit is dirty', async () => {
    ...
    act(() =>
      result.current.form.setValue('name', 'Nome alterado', { shouldDirty: true }),
    )
  ...
  it('confirms before clearing legal associations when switching to global scope', () => {
    ...
    act(() => result.current.handleAreaToggle('area-1'))
    act(() => result.current.handleTopicToggle('area-1', 'topic-1'))
    act(() => result.current.handleApplicationScope('global'))
  ...
  it('allows an unavailable edit to save an empty template', async () => {
    ...
    expect(updateConfiguration).toHaveBeenCalledWith({
      id: 'spec-1',
      request: expect.objectContaining({ status: 'unavailable' }),
    })
  })
