...
export function useDocumentSpecificationPage({
  ...
  async function handleDeleteConfirm() {
    ...
    window.location.replace('/modelos-de-documentos')
  }
  ...
  function handleRemoveVariable(variable: DocumentTemplateVariable) {
    ...
    setVariables((previous) =>
      systemTechnicalName
        ? previous.map((current) =>
            current.systemTechnicalName === systemTechnicalName
  ...
  async function handleTemplateSave() {
    if (mode === 'create') {
      ...
      await createAndSaveModel()
      return
    }
    if (!documentSpecificationId || (!isConfigurationDirty && !isTemplateDirty)) return

    const shouldSaveConfiguration = isConfigurationDirty
    ...
    if (shouldSaveConfiguration) {
      const saved = await saveConfiguration(watchedConfiguration)
      if (!saved) return
    }
    const message = shouldSaveConfiguration
      ? 'Template e configuração salvos com sucesso.'
  ...
  async function createAndSaveModel() {
    ...
    const templateResponse = await updateTemplate({
      id: response.body.documentSpecificationId,
      request: { content, variables },
    })
    if (templateResponse.isFailure) {
      toast.error(templateResponse.errorMessage)
      return
    }
    isNavigatingAfterSave.current = true
    window.location.replace(
  ...
  const canSaveModel = isConfigurationReady && !isTemplateEmpty && !isCatalogError
