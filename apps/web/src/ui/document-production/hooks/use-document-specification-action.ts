...
export const useDocumentSpecificationActions = () => {
  ...
  const remove = useMutation({
    mutationFn: (id: string) => documentProductionService.deleteDocumentSpecification(id),
    onSuccess: (_data, id) => {
      void queryClient.invalidateQueries({ queryKey: ['document-specifications'] })
      void queryClient.removeQueries({ queryKey: ['document-specification', id] })
    },
  })
  ...
  return {
    ...
    deleteDocumentSpecification: remove.mutateAsync,
