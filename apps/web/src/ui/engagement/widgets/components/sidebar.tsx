...
export const Sidebar = ({ isCollapsed, setIsCollapsed, activePath, setActivePath, sidebarItems }: SidebarProps) => {
    return (
     <aside
      ...
      >
        ...
        <div className="w-full flex flex-col items-start px-4 pt-4 gap-6">
          ...
          <div className={`relative flex w-full items-center ${isCollapsed ? 'flex-col gap-3 justify-center' : 'justify-center min-h-[64px]'}`}>
            {!isCollapsed ? (
              <>
                <div className="flex flex-col bg-white rounded p-4 items-center justify-center w-24">
                  <span className="font-serif font-bold text-brand text-lg tracking-wide">HMS</span>
          ...
          <nav className="flex flex-col w-full gap-2">
            {sidebarItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={`w-5 h-5 shrink-0 ${isActive ? 'text-highlight-vivid' : ''}`} />
