...
export const Stepper = ({ steps, activeStep, orientation = 'horizontal' }: StepperProps) => {
            ...
            <div key={idx} className="relative flex flex-col gap-1">
              ...
              >
                {isCompleted ? (
                  ...
                  <div className={`size-2 rounded-full ${isActive ? 'bg-brand-highlight' : 'bg-transparent'}`} />
                )}
              </div>
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <h4 className={`text-sm font-semibold ${isActive ? 'text-brand-highlight' : 'text-foreground'}`}>
                  {step.label}
                </h4>
                {step.description && (
        ...
        return (
          <div key={idx} className="flex flex-col items-center text-center relative z-10 flex-1">
            ...
            <div className="mt-3 px-2">
              <span className={`text-xs md:text-sm font-medium block ${isActive ? 'text-brand-highlight' : 'text-foreground'}`}>
                {step.label}
              </span>
              {step.description && (
