...
interface AddFactDialogProps {
  isOpen: boolean
  onClose: () => void
  onAdd: (fact: { id?: string; date: string; description: string; status: string }) => void
  factToEdit?: FactToEdit | null
}

const MAX_DESCRIPTION_LENGTH = 1000

export function AddFactDialog({ isOpen, onClose, onAdd, factToEdit }: AddFactDialogProps) {
  ...
  const handleSubmit = () => {
    ...
    const result = relevantFactSchema.safeParse({
      description,
      date: formattedDate !== '—' && formattedDate !== 'Indefinida' ? formattedDate : null,
    })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xl w-full max-w-md p-6 space-y-5">
        ...
        <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
          ...
          >
            <Plus className="w-3.5 h-3.5" /> {factToEdit ? 'Salvar alterações' : 'Adicionar fato'}
          </Button>
        </div>
      </div>
    </div>
