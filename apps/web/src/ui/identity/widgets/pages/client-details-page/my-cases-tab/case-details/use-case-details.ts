...
import { useIntakeDetailsQuery } from '@/ui/identity/hooks/use-intake-details-query'

export function useCaseDetails() {
  const { caseId } = useParams({ from: '/cliente/meus-casos/$caseId' })
  const { navigateTo } = useNavigation()
  const { caseDetails, caseDetailsError, isLoadingCaseDetails } =
    useIntakeDetailsQuery(caseId)

  // Mapping IntakeStatus to stepper index
  function getActiveStepIndex(status?: IntakeStatus): number {
    if (!status) return 0
    switch (status) {
  ...
  function getDocuments() {
    return [
      ...
      },
      {
        ...
        status: activeStep >= 3 ? 'approved' : 'pending',
        updatedAt: '2026-07-29',
      },
  ...
  return {
    ...
    activeStep,
    documents,
    pendingDocuments: documents.filter((document) => document.status === 'pending'),
