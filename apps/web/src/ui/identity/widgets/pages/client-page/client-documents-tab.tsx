    ...
    <Card className="flex flex-col border border-border rounded-xl bg-card overflow-hidden shadow-sm mb-4">
      ...
      >
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          <span className="font-mono tracking-wide text-foreground font-medium">{batch.readableId}</span>
            ...
            batch.files.map((file: any) => (
              <div key={file.id} className="flex items-center justify-between px-5 py-4 border-b border-border last:border-0">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <Button variant="ghost" size="icon-sm" className="text-muted-foreground" onClick={() => navigate({
      ...
      <div className="flex flex-col">
        {batches.length === 0 ? (
          ...
          batches.map((batch: any) => (
            <DocumentBatchCard key={batch.id} batch={batch} />
          ))
        )}
      </div>
    </div>
