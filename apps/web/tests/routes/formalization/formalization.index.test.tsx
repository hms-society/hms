...
test.describe('document package confirmation flow', () => {
  ...
  test('confirms the document package and closes without contract with the selected reason', async ({
    ...
    await expect(packageDialog).toContainText(
      'não inicia assinaturas nem envia mensagens',
    )
    ...
    await expect(
      page.getByRole('link', { name: 'Configuração do envio' }),
    ).toHaveAttribute('href', `/formalizacoes/${FORMALIZATION_ID}/configuracao-envio`)
    await expect(page.getByRole('button', { name: 'Confirmar contratação' })).toHaveCount(
      1,
    )
    expect(
