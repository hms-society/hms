...
ALTER TABLE "document_specifications_audit_logs" ADD CONSTRAINT "document_specifications_audit_logs_document_specification_id_document_specifications_id_fk" FOREIGN KEY ("document_specification_id") REFERENCES "public"."document_specifications"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "case_members_one_primary_per_case_uidx" ON "case_members" USING btree ("case_id") WHERE "case_members"."is_primary" = true;--> statement-breakpoint
...
CREATE INDEX "cases_opened_at_idx" ON "cases" USING btree ("opened_at");