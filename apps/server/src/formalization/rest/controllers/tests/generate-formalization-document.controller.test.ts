import request from 'supertest'
import { afterAll, beforeAll, describe, expect, it } from 'vitest'

import { FormalizationModuleFixture } from '@/formalization/fixtures'

describe('Generate Formalization Document Controller [POST /formalizations/:formalizationId/documents/:documentId/generations]', () => {
  let fixture: FormalizationModuleFixture

  beforeAll(async () => {
    fixture = await FormalizationModuleFixture.register()
  })

  afterAll(async () => fixture?.close())

  it('rejects an invalid Formalization identifier at the HTTP boundary', async () => {
    const response = await request(fixture.app.getHttpServer())
      .post(
        '/formalizations/not-a-uuid/documents/00000000-0000-4000-8000-000000000001/generations',
      )
      .send({})

    expect(response.status).toBe(400)
  })
})
