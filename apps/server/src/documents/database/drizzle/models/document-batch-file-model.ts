import { pgTable, text, timestamp, uuid, integer, jsonb, boolean, AnyPgColumn } from 'drizzle-orm/pg-core'
...
export const documentBatchFileModel = pgTable('document_batch_files', {
  ...
  batchId: uuid('batch_id').references(() => documentBatchModel.id, { onDelete: 'cascade' }).notNull(),
  ...
  originalDocumentId: uuid('original_document_id').references((): AnyPgColumn => documentBatchFileModel.id, { onDelete: 'set null' }),
  aiSuggestion: jsonb('ai_suggestion'),
  humanCorrection: jsonb('human_correction'),
  reviewedBy: uuid('reviewed_by').references(() => userModel.id, { onDelete: 'set null' }),
