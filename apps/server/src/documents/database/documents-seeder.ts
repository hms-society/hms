...
export class DocumentsSeeder {
  ...
  async run() {
    ...
    const clients = await db.select().from(clientModel).limit(1)
    ...
    const dummyFiles = [
      { name: 'contrato_social_simulado.pdf', content: 'PDF_DUMMY_CONTENT_123', mime: 'application/pdf' },
      { name: 'comprovante_endereco_simulado.jpg', content: 'JPG_DUMMY_CONTENT_456', mime: 'image/jpeg' },
    ]
    ...
    const uploadedFiles = await Promise.all(
      dummyFiles.map(async (file) => {
        ...
        const storagePath = `seed/${userId}/${timestamp}-${file.name}`
