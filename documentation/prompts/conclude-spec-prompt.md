...
- **Contract change:** set the Spec to `draft`, immediately invoke `create-spec`, obtain required
  product or technical authority, update the canonical Confluence PRD first when applicable
  and authorized, increment the revision, reconcile Plan/evaluation and continue
...
Never change Jira or Confluence state merely because closure reopened. External mutations require
explicit user authorization and follow their own product/delivery workflow.
...
- the canonical Evaluation template embedded in `implement-spec`, including its sections,
...
- propose or, when explicitly authorized, update the canonical Confluence PRD when the finding
  clarifies product behavior, user-visible states, permissions, or acceptance intent;
...
review feedback is handled by `resolve-pr-pendencies`; while the PR remains open, that workflow
