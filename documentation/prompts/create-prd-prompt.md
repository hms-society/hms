...
For every requirement, use the canonical subsection titles `Regras de negócio` for business
rules and `Regras de UI/UX` for presentation and interaction rules. Do not use `Capacidades` or
`Experiência` as requirement subsection titles. Omit a subsection when it has no applicable
rules instead of creating an empty placeholder.

Never add an implementation-status field or checkbox such as `Implementado` or
`Não implementado` to a requirement. Implementation progress belongs in Jira and the delivery
artifacts, not in the PRD.
