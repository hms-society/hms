...
3. Verify every affected path and change classification against the repository. Check exact
   ...
   inputs/outputs, commands, and prohibited paths against the selected Rules. Reject any
   affected-path table whose first two columns are not exactly `Path | Change`, whose row names
   anything other than one backticked repository-relative file, whose change is not exactly
   `Create`, `Modify`, `Generate`, or `Remove`, or whose path is duplicated elsewhere in the
   affected-path maps.
...
- Do not waive malformed or noncanonical affected-path tables; report them as blocking because
  the implementation path gate cannot evaluate the Contract reliably.
