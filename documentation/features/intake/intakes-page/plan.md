...
| RF-02 | Criar os ports read-only de Identidade em `packages/core/src/identity/interfaces/` para resolver Clientes por nome/documento, ler resumos mascarados e listar opções mínimas de responsáveis; atualizar `packages/core/src/intake/interfaces/intake-service.ts` com `listIntakes` e `listIntakeResponsibles`. | Os contratos permitem composição por IDs, filtro de responsável e endpoint de opções sem importar models ou repositories de outro módulo. | `REQ-04`, `REQ-07`, `REQ-12`, `CA-05`, `CA-09`, `CA-19` | Não |
