...
- **Spec:** [`spec.md`](./spec.md), revision `13`, status `complete`; it aggregates every current worktree change under the explicit accepted-risk conclusion waiver.
...
- **Current phase:** `F8` — final PR CI gate — `completed`; PATH-03 covers every current worktree change under revision 13 and EV-239 records green Core/Server/Web/check-size checks on all 19 delivery heads.
- **Next action:** Handoff the green PR stack for review/merge; no further implementation or CI correction is pending in this Spec.
- **Active blockers:** MV-01–MV-08, remaining visual evidence, G-01/G-02/G-05/G-06 and FND-047/FND-049/FND-050/FND-052 remain explicitly accepted risks, not passed or technically resolved. They do not block this conclusion under EV-235. FND-058 is resolved by EV-239.
...
| 11 | `orchestrator` | F8 | Revision-13 conclusion preflight and PR CI gate | F13 | — | `completed` | PATH-03 passes with 1,282 declared/current paths, zero failed checks and zero unrelated changes; all 19 PR heads pass applicable Core/Server/Web/check-size checks. |
...
- **Contract:** FR-01–FR-27 and FR-29–FR-38; AC-01–AC-15 and AC-17–AC-22; preserve the exact state tables, one-email channel, request graph, atomic projections, token/OTP/session rules, provider ports and event declarations from the Spec.
...
- **Contract:** FR-01–FR-05, FR-09–FR-14, FR-26–FR-27 and FR-29–FR-38; AC-01–AC-05, AC-08, AC-14–AC-15 and AC-17–AC-22; strict unknown-key rejection, six-digit OTP, one-email channel and absence of provider secrets/links.
...
- **Contract:** FR-09–FR-25 and FR-31–FR-38; AC-09–AC-14 and AC-18–AC-21; enforce one request/document/recipient graph, one email delivery channel, hashed/encrypted secret boundaries, leases, dedupe, version checks, advisory/row locks and atomic `signatureStatus` projections.
...
- **Contract:** FR-12–FR-14, FR-18–FR-25, FR-28 and FR-31–FR-38; AC-08, AC-10–AC-14, AC-16 and AC-18–AC-21; G-04 provider pin, private PostgreSQL/Documenso, Resend version, certificate/runtime defaults and no route exposure.
...
- **Contract:** FR-03–FR-25 and FR-28–FR-38; AC-02–AC-14, AC-16 and AC-18–AC-21; real request/response, authorization, cookie/CSRF/Origin, provider alias/proxy, Resend, Inngest retry/lease, webhook/reconciliation, artifact and cancel-all behavior.
...
- **Contract:** FR-29–FR-38; AC-17–AC-22; review of immutable PDFs/assignments/email/message, confirm-once, progress/partial failure, locked editing, cancel-all, safe retry and preserved history.
...
- **Contract:** FR-01–FR-27; AC-01–AC-16; fragment-only exchange, safe return target, client email OTP, collaborator session, private PDF, acknowledgement, provider handoff, submission/result states, no raw token/provider URL and all unavailable/locked/reconciliation states.
...
- **Contract:** FR-06–FR-25 and FR-31–FR-38; AC-04–AC-14 and AC-18–AC-22.
...
- **Contract:** FR-01–FR-19 and FR-31–FR-38; AC-01–AC-15 and AC-18–AC-22.
...
- **Contract:** FR-03–FR-25 and FR-28–FR-38; AC-02–AC-14 and AC-16–AC-22.
...
- **Contract:** FR-01–FR-15 and FR-26–FR-27; AC-01–AC-15. Design Contract states `reading` from `design/manifest.md`, nodes UGhOX/Cl6te amended by revision 6, and references `04-read-document.png`, `05-confirm-signature.png`, `09-collaborator-read.png` and `10-collaborator-confirm.png`.
...
- **Status/owner:** `completed` — `orchestrator` (PATH-03 and final PR CI gate pass; EV-239 records all 19 green delivery heads)
...
- **Contract:** Full FR-01–FR-38, AC-01–AC-22, MV-01–MV-08 and G-01–G-06 conformance; cross-Builder contracts, security/privacy, persistence, REST, jobs, route/widget tree and documentation alignment.
...
| --- | --- | --- | --- | --- | --- |
| Automated/runtime | MV-01 — local automated lifecycle, races and persistence | AC-01–AC-22 | Spec MV-01 and quality commands | `./evaluation.md` with command output and database evidence | `waived` |
| Runtime | MV-02 — local full stack client flow | AC-01–AC-14, AC-17–AC-20 | Spec MV-02 | `./evaluation.md` plus transient trace/screenshots and real request/response/persistence evidence | `waived` |
| Runtime | MV-03 — local real Auth collaborator flow | AC-04, AC-06, AC-15 | Spec MV-03 | `./evaluation.md` plus authenticated Playwright trace, screenshot, console and failed-request classification | `waived` |
| Runtime | MV-04 — failure and recovery | AC-09–AC-14, AC-21 | Spec MV-04 | `./evaluation.md` with controlled outage, retry, revocation, no-duplicate and reconciliation evidence | `waived` |
| Security | MV-05 — token-boundary canary | AC-01, AC-02, AC-09, AC-14–AC-15 | Spec MV-05 | `./evaluation.md` with zero-match scan across URL/DOM/network/storage/cookies/console/trace/screenshots/logs | `waived` |
| Visual/accessibility | MV-06 — keyboard, screen reader smoke, responsive, zoom, themes and motion | AC-15, AC-22 | Spec MV-06 and design manifest | `./evaluation.md` with Playwright CLI screenshots, focus/live-region and console/request evidence | `waived` |
| Release/runtime | MV-07 — staging Resend, provider privacy, X.509 and pinned runtime | AC-08, AC-10–AC-14, AC-16, AC-20 | Spec MV-07; G-01/G-02/G-04/G-05 | `./evaluation.md` with transient staging artifact identifiers and gate sign-offs | `waived` |
| Security/operations | MV-08 — AGPL, scans, drift, alerts and redacted operations | AC-14, AC-16 | Spec MV-08; G-01/G-02/G-04/G-05 | `./evaluation.md` with security review, scan, alert and runbook evidence | `waived` |
...
| Type | Scenario/surface | Criteria | Reference | Evidence target | Status |
| --- | --- | --- | --- | --- | --- |
| Visual | Internal configuration baseline | AC-17, AC-22 | `./design/qOfh6.png` | `./evaluation.md` — source desktop and 390×844 captures | `waived` |
| Visual | Ready-to-send action card | AC-17, AC-22 | `./design/YWfhi.png` | `./evaluation.md` — source desktop and 390×844 captures | `waived` |
| Visual | Add-signatory regression | AC-17 | `./design/sxENj.png` | `./evaluation.md` — source desktop and 390×844 captures | `waived` |
| Visual | Candidate selection regression | AC-17 | `./design/Vx43H.png` | `./evaluation.md` — source desktop and 390×844 captures | `waived` |
| Visual | Signature-field positioning regression | AC-17, AC-19 | `./design/HcT8k.png` | `./evaluation.md` — source desktop and 390×844 captures | `waived` |
| Visual | Review/confirm dialog | AC-17–AC-20 | `./design/nI1B0.png` | `./evaluation.md` — source desktop and 390×844 captures | `waived` |
| Visual | Provisioning/delivery/partial failure | AC-18–AC-22 | `./design/MC4E2.png` | `./evaluation.md` — source desktop and 390×844 captures | `waived` |
| Visual | Cancel-all dialog | AC-21–AC-22 | `./design/NSYug.png` | `./evaluation.md` — source desktop and 390×844 captures | `waived` |
| Visual | Invitation access | AC-01, AC-15 | `./design/01-access-invitation.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Channel confirmation | AC-02, AC-15 | `./design/02-select-channel.png` | `./evaluation.md` — 1200×900 and 390×844 captures; email-only revision-5 structure | `waived` |
| Visual | OTP verification | AC-03, AC-15 | `./design/03-verify-otp.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Private document reading | AC-04, AC-06–AC-07, AC-15 | `./design/04-read-document.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Multi-document tabs, per-document acknowledgement and all-read gate | AC-06–AC-08, AC-15 | Revision-6 amendment of `./design/04-read-document.png` and `./design/05-confirm-signature.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Submitted/reconciliation | AC-09–AC-10, AC-15 | `./design/06-signature-submitted.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Confirmed result | AC-11–AC-13, AC-15 | `./design/07-signature-confirmed.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
...
| Visual | Collaborator document reading | AC-04, AC-06, AC-15 | `./design/09-collaborator-read.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Collaborator acknowledgement | AC-06–AC-07, AC-15 | `./design/10-collaborator-confirm.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Generic unavailable state | AC-15 | `./design/11-access-unavailable.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | No authorized email channel | AC-02, AC-15 | `./design/12-no-authorized-channel.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Invalid/expired OTP | AC-03, AC-15 | `./design/13-otp-invalid-expired.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | OTP attempt lock | AC-03, AC-15 | `./design/14-otp-attempt-limit.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Private PDF temporarily unavailable | AC-07, AC-15 | Manifest derived state from `./design/11-access-unavailable.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Signing provider temporarily unavailable | AC-08, AC-15 | Manifest derived state from `./design/11-access-unavailable.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Recipient rejected | AC-13, AC-15 | Manifest derived state from `./design/11-access-unavailable.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Request/recipient cancelled | AC-13, AC-15, AC-21 | Manifest derived state from `./design/11-access-unavailable.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Invitation/session/result expired | AC-01, AC-03, AC-09, AC-15 | Manifest derived state from `./design/11-access-unavailable.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Collaborator inactive/ineligible/unassigned | AC-04, AC-06, AC-15 | Manifest derived state from `./design/11-access-unavailable.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
| Visual | Confirmation reconciliation required | AC-10–AC-13, AC-15 | Manifest derived state from `./design/11-access-unavailable.png` | `./evaluation.md` — 1200×900 and 390×844 captures | `waived` |
