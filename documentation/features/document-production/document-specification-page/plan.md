...
|---|---|
| Plan | `completed` |
| Spec | `completed`, revisão 3 |
| Fase atual | Quality Gate e aceite final concluídos |
| Próxima ação | handoff para commit/PR quando solicitado |
| Judge Plan | `accepted` |
| Judge Implementation | `accepted` — único Judge da implementação inteira |
...
registro ao abrir ou abandonar a rota; o administrador preenche configuração e
template e confirma **Salvar modelo**. A operação persiste a identidade com o
status escolhido — **Disponível** por padrão —, configuração, template e
associações, com autorização, atomicidade, validação e feedback completos.

## Escopo

Inclui:

- contratos Core, schemas compartilhados, erros e cinco casos de uso;
...
- repository, mapper, seed, composição Nest e cinco endpoints REST protegidos;
...
- duplicar modelos e alterar a ação **Duplicar**;
...
| `vBrek` | Detalhe desktop — aba Configuração | Nome e descrição; status disponível/indisponível; aplicação global/jurídica; áreas e temas; indicador de salvo | Configuração corresponde ao frame desktop, com descrição opcional e sem mutações de sidebar/navbar de contexto |
...
| `FQtUK` | Detalhe desktop — aba Template vazia | Estado vazio; orientação **Começar a escrever**; instrução de colagem | A ação foca o editor sem inserir conteúdo fictício; modelo disponível exige conteúdo, enquanto modelo indisponível pode salvar somente a configuração |
...
`vBrek` marca a descrição como opcional e esta revisão adota essa decisão.

### Estado no início da revisão 2 (histórico de descoberta)
...
|---|---|---|---|
| F1 — Core e validação | `verified` | Judge Plan aceito | contratos, schemas e casos de uso passam seus sensores |
| F2 — Persistência, migration e seed | `verified` | F1 verificada | JSONB, backfill, transações, mapper e fixture funcionam |
| F3 — REST e autorização | `awaiting_judgment` | F2 verificada | POST/GET/PATCH retornam projeção correta e `401/403/404/400` |
| F4 — Web, rotas e experiência | `verified` | F3 verificada | sensores web e integração concluídos; sem Judge intermediário |
| F5 — Integração e Quality Gate | `accepted` | F1–F4 sensores verificados | browser real, gate global e único Judge final aceitos |
...
| F1-T2 — Casos de uso e regras de domínio | `verified` | `packages/core/src/document-production/use-cases/{create,get,update-document-specification-configuration,update-document-specification-template,delete-document-specification}-use-case.ts`; `use-cases/tests/**`; fakers/fixtures do domínio | criação normaliza textos, aceita descrição vazia, força `available` por padrão, valida template e catálogo; get, updates e delete convertem ausência; configuração e template preservam dados não alterados; template valida árvore, texto e tokens | RF-001–RF-009, RF-011–RF-012; CA-01–CA-09, CA-12, CA-15, CA-18–CA-22, CA-26–CA-32 | sim após F1-T1; não compartilha paths de produção com F1-T3 |
| F1-T3 — Schemas compartilhados e testes | `verified` | `packages/validation/src/document-production/schemas/**`; `schemas/tests/**`; barrels e `package.json` | schemas Zod estritos rejeitam payloads extras, validam união global/jurídica, árvore recursiva, links HTTP(S), variáveis e os três bodies sem aceitar status/content/identity no create | RF-003–RF-006, RF-009, RF-011; CA-07–CA-09, CA-13–CA-15, CA-21, CA-28–CA-29 | sim após F1-T1; independente de F1-T2 |
...
com uma ação principal de salvamento, fronteiras internas preservadas e editor
JSON rico.

| Tarefa | Estado | Paths principais | Resultado observável | Traceabilidade | Parallelizable |
|---|---|---|---|---|---|
| F4-T1 — Dependência, adapter e rotas | `verified` | `apps/web/package.json`; `pnpm-lock.yaml`; `apps/web/src/rest/services/document-production-service.ts` e testes; `apps/web/src/constants/routes.ts`; `apps/web/src/routes/modelos-de-documentos/{index.tsx,novo.tsx,$documentSpecificationId.tsx}`; `apps/web/src/routeTree.gen.ts` | Tiptap v3 é adicionado somente ao web; service mapeia POST/GET/PATCHes sem auth/negócio; criação é client-only e protegida; detalhe usa UUID semântico e `AppLayout`; listagem conecta **Novo modelo**/**Editar**, preserva **Duplicar** fora de escopo e invalida/navega corretamente | RF-001, RF-008, RF-010, RF-011–RF-012; CA-01–CA-02, CA-18–CA-20, CA-22–CA-32 | não; rotas, constants e route tree são pontos compartilhados |
| F4-T2 — Página, forms, catálogo e editor | `verified` | `apps/web/src/ui/document-production/widgets/pages/document-specification-page/**`; hooks/actions/query; testes colocados em `tests/` | página create/edit, React Hook Form + Zod, uma ação de salvamento para configuração/template, dirty guard, loading/error/404/retry; editor Tiptap com `immediatelyRender: false`, JSON estrito, toolbar, links HTTP(S), variáveis e busca | RF-002–RF-010, RF-011–RF-012; CA-03–CA-20, CA-23–CA-32 | não; depende de F4-T1 e compartilha contratos de view |
| F4-T3 — Navegação, estados e integração de rota | `verified` | `apps/web/src/ui/document-production/widgets/pages/document-specifications-page/**`; `apps/web/src/routes/modelos-de-documentos/index.test.tsx`; `apps/web/tests/routes/document-production/**` | **Novo modelo** abre `/modelos-de-documentos/novo` sem POST; **Editar** abre o ID; transport mockado stateful cobre create `201`, GET/PATCH/delete, redirect replace, filtros de catálogo, erros, not-found, pending, dirty guard e re-render sem requests duplicados | RF-001, RF-002, RF-008, RF-010, RF-011–RF-012; CA-01–CA-04, CA-10–CA-20, CA-23–CA-32 | não; depende de F4-T2 e da rota gerada |
...
|---|---|---|---|---|---|
| F5-T1 — Preflight de dependências | `verified` | `evaluation.md`: Docker/Auth/banco saudáveis, health do server `:3333` e bootstrap Nest confirmado | dependências locais saudáveis, sem `UnknownDependenciesException` | RF-001, RF-009; CA-01–CA-02, CA-22, CA-25–CA-30 | não; bloqueia browser |
| F5-T2 — Fluxo browser autenticado real | `verified` | `evaluation.md`: Playwright MCP autenticado e integração mockada; console/network classificados | login/sessão admin, listagem, create/redirect, GET/PATCH reais, editor/variáveis, dirty guard, teclado, viewport estreito e gates visuais verificados | todos RF; CA-01–CA-32 | não; depende de F5-T1 e F1–F4 |
| F5-T3 — Sensores integrados e Quality Gate | `verified` | `evaluation.md`: format, lint, types, testes, build e integração repetidos após correções | sensores integrados verdes; warnings/notices não bloqueantes classificados | todos RF/CA | não; gate global |
| F5-T4 — Judge Implementation Final único | `accepted` | `evaluation.md`: Judge `019fe76d-f32e-7f03-bdcd-aa9cd03f6b07` | um único Judge aceitou a implementação inteira, sem findings pendentes | todos RF/CA | não; etapa final |
...
cinco nodes Pencil. Registrar a decisão normativa de descrição opcional, status
inicial Disponível e a derivação do modo vazio sem inventar Node ID.
...
| RF-008 / CA-18–20 | F1-T1/T2, F2-T2/T3, F3-T1/T2, F4-T2/T3, F5-T2 | uma ação principal, fronteiras internas, atomicidade e dirty guard |
...
| RF-012 / CA-31–32 | F1-T2, F2-T2, F3-T1/T2, F4-T2/T3, F5-T2 | confirmação, delete transacional e retorno à listagem |
...
| R-007 | duas fronteiras de rascunho | aberto | salvar modelo pode sobrescrever template ou vice-versa | refs salvas independentes, operação coordenada, pending/dirty tests e browser | validar em F3-T2/F4-T2/T3 |
...
| R-009 | divergência visual/documental | resolvido | PRD versão 7, Spec revisão 3 e Plan estão alinhados; permanecem apenas os gates técnicos e visuais | manter evidências do Pencil e Playwright no encerramento | concluir em F5-T2 |
...
Os vereditos abaixo são evidências históricas da revisão 2. A decisão de negócio
confirmada em 09/08/2026 alterou o Contract; portanto, o aceite histórico não é
válido para esta revisão 3. A implementação deve receber um novo veredito único
somente após o rebaseline e o Quality Gate completo.
...
phases:
  ...
  F4:
    status: accepted
    findings: []
    next_action: Quality Gate final
  F5:
    status: accepted
    verdict: accepted
    findings: []
    next_action: handoff para commit/PR
```
...
### Encerramento final — revisão 3

status: completed
verdict: accepted
single_judge:
  agent: 019fe76d-f32e-7f03-bdcd-aa9cd03f6b07
  scope: implementação inteira da página
  status: accepted
  findings: []
evidence:
  - Quality Gate: lint, tipos, testes, build, Biome e diff check passed
  - Pencil: `K2Fvp`, `vBrek`, `V7lxA`, `FQtUK` e `fRdNH` validados
  - Playwright autenticado: criação, edição, template, variáveis, dirty guard,
    viewport estreito, tema escuro/reflow, confirmação de aplicação e remoção
    real; console/network final limpos
  - testes finais: Core 25/114, Validation 7/19, server 7/25 e Web 39/168
