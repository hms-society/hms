...
| [`jobs-testing-rules.md`](jobs-testing-rules.md) | Creating or changing Inngest job tests, module-fixture job composition, Dockerized job infrastructure, job business-rule coverage, or the Server integration suite. | `apps/server/src/**/messaging/inngest/jobs/tests/**`, `apps/server/src/shared/messaging/inngest/inngest-fixture.ts`, `apps/server/vitest.integration.config.mts`, server CI `test:integration` step |
...
| Add a web REST service for an existing endpoint | UI Layer + REST Layer; verify the mapping through its consuming widget/route boundary; add Core Package when the service contract changes |
