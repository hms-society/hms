...
4. [`rules/rules.md`](rules/rules.md), followed by every dynamically selected Rule;
...
| --- | --- | --- |
| Orchestrator | Selects workflows, owns artifact state, researches Specs directly, creates Spec Reviewer/implementation/review subagents, integrates diffs, runs sensors, records evidence, publishes the PR, and routes failures. | Does not delegate Spec research or authoring, the official evidence verdict, required sensors, or claimed evidence. |
...
| [Implementation Reviewer](agents/implementation-reviewer-agent.md) | Independently audits one integrated Plan-backed candidate. | Does not edit, implement fixes, or decide the official readiness verdict. |

Spec Reviewer, Builder, and Implementation Reviewer subagents are siblings created by the
Orchestrator in the current task. No subagent creates another subagent, user-owned task, fork,
or handoff.
...
flowchart TD
    ...
    D --> IN["Integrate Builder and Orchestrator-owned paths"]
    F --> IN
    IN --> P["Structural path gate"]
    P --> V{"Plan-backed?"}
    V -->|No| G["Integrated sensors and Playwright CLI evidence"]
    V -->|Yes| R["reviewer and integrated sensors"]
...
or approval verdict. A valid Spec moves to `open` and recommends either direct execution or a
Plan.

Every implementation-facing Spec uses strict canonical affected-path tables. Each such table
has `Path | Change` as its first two columns, may add layer-specific columns after them, and has
one repository-relative exact file path per body row. Directory paths, globs, path ranges,
comma-separated files, shorthand, and multiple files in one cell are not canonical. `Change`
is exactly one of `Create`, `Modify`, `Generate`, or `Remove`, verified against the authoring
baseline. Every affected file appears exactly once across the tables. A malformed or
noncanonical affected-path table keeps a draft Spec from becoming `open`.

This contract applies when creating or materially amending an active Spec. Do not bulk-normalize
existing feature Specs solely to adopt the table shape; reconcile an open or resumed legacy
artifact only through its normal migration or amendment workflow.
...
corrections. A material ambiguity returns to the create-spec clarification gate. The Spec may
become `open` only when the applicable review is current and every verified finding is resolved.
...
6. record the exact command, selected base ref, resolved base SHA, reported counts, and result in
   Evaluation before integrated sensors or readiness work;
7. for Plan-backed work, activate exactly one read-only subagent named `reviewer` only after the
   structural path gate passes;
8. run repository-approved integrated sensors, verify findings, invalidate stale evidence,
   resume the responsible Builder, and rerun affected validation until Evaluation is `ready`.
...
The structural path gate uses local `origin/develop` by default, performs no automatic fetch,
and accepts an explicit `--base <ref>` when another local baseline governs the delivery. `--json`
requests machine-readable output without changing the checks. It is a structural comparison of
canonical affected-path rows to changed implementation paths, not behavioral, test, runtime,
design, accessibility, generated-content, or migration validation. Rerun it after every
contracted-path correction. The Implementation Reviewer consumes the current passing Evaluation
row and does not substitute an independent rerun.
...
Authenticated UI validation follows the root `AGENTS.md` Playwright CLI workflow: verify local
...
For Plan-backed execution, the same `reviewer` subagent rechecks corrected candidates using the
[`Implementation Reviewer`](agents/implementation-reviewer-agent.md) contract. The
...
the structural path gate has a current passing Evaluation row, and no blocking finding remains.

With explicit authorization to commit, push, and publish, conclusion:

1. reruns the structural path gate against the selected local base, records the exact command,
   base ref/SHA, counts, and passing result, then runs local closure preflight and final
   Spec-to-diff conformance;
