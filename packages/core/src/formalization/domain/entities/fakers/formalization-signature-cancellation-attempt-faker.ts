...
export function fakeFormalizationSignatureCancellationAttempt(overrides: Partial<FormalizationSignatureCancellationAttempt> = {}): FormalizationSignatureCancellationAttempt {
  ...
  return {
    ...
    requestId: faker.string.uuid(), attemptToken: faker.string.uuid(), status: 'pending', attempts: 0, requestedBy: faker.string.uuid(), reason: 'Solicitação de cancelamento', requestedAt: now, updatedAt: now,
    ...overrides,
  }
}
