  ...
  implements UseCase<Request, DocumentVersion>
{
  ...
  async execute(request: Request): Promise<DocumentVersion> {
    ...
    if (!version) throw new FormalizationStateConflictError('A versão documental não foi encontrada.')
    ...
    if (!packageDocuments.some((document) => document.documentId === version.documentId)) {
      throw new FormalizationStateConflictError('A versão documental não pertence à formalização.')
    }
    return version
  }
}
