    ...
    const updated = await this.specificationsRepository.replaceTemplate(
      documentSpecificationId,
      {
        ...
        variables: changes.variables.map((variable) => ({
          ...
          ...(variable.systemTechnicalName?.trim()
            ? { systemTechnicalName: variable.systemTechnicalName.trim() }
            : {}),
          ...(variable.isRemoved ? { isRemoved: true } : {}),
  ...
  private assertBlock(node: unknown): void {
    ...
    if (block.type === 'paragraph' || block.type === 'heading') {
      if (block.type === 'heading') {
        ...
        !['left', 'center', 'right', null].includes(
    ...
    if (
      ...
      for (const child of block.content)
        block.type === 'blockquote' ? this.assertBlock(child) : this.assertListItem(child)
      return
    }
    throw new InvalidDocumentTemplateError()
  }
