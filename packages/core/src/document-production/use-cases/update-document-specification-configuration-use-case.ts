  ...
  implements UseCase<Request, DocumentSpecification>
{
  ...
  async execute({
    ...
    if (changes.status === 'available') {
      this.assertValidTemplate(current.content, current.variables)
    }
  ...
  private assertBlock(node: unknown): void {
    ...
    if (block.type === 'blockquote' || block.type === 'bulletList') {
      ...
      for (const child of block.content) {
        if (block.type === 'bulletList') this.assertListItem(child)
        else this.assertBlock(child)
      }
      return
    }
    throw new InvalidDocumentTemplateError()
  ...
  private assertTextBlock(
    ...
    if (isHeading) {
      ...
      if (!['left', null].includes(attrs.textAlign as null | string)) {
        throw new InvalidDocumentTemplateError()
      }
    }
