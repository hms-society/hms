...
import type { Client, Collaborator } from '../domain/entities'
import {
...
describe('UpdateClientUseCase', () => {
  ...
  let clientsRepositoryMock: Mocked<ClientsRepository>
  let usersRepositoryMock: Mocked<UsersRepository>
  let collaboratorsRepositoryMock: Mocked<CollaboratorsRepository>
  ...
  beforeEach(() => {
    ...
    } as unknown as Mocked<ClientsRepository>
    ...
    } as unknown as Mocked<CollaboratorsRepository>
  ...
  it('deve bloquear Atendente tentando alterar CPF ou Nome', async () => {
    const changes = {
      type: 'natural',
      name: 'João Alterado',
      taxId: { type: 'cpf', value: '99999999999' },
    }
  ...
  it('deve permitir Atendente alterar E-mail e Endereço', async () => {
    ...
    clientsRepositoryMock.replace.mockResolvedValueOnce({
      ...mockClient,
      ...changes,
    } as Client)
  ...
  it('deve disparar erro 409 se CPF já existir em outro cliente', async () => {
    ...
    clientsRepositoryMock.findByTaxId.mockResolvedValueOnce({
      id: 'outro-cliente',
    } as Client)
