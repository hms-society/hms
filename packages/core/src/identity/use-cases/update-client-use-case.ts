...
export class UpdateClientUseCase implements UseCase<Request, Client> {
  ...
  async execute({
    ...
    if (isAttendant) {
      const isNameChanged =
        changes.type === 'natural' &&
        (changes as any).name !== undefined &&
        (changes as any).name !== (client as any).name
      const isLegalNameChanged =
        changes.type === 'legal' &&
        (changes as any).legalName !== undefined &&
        (changes as any).legalName !== (client as any).legalName
      const isTradeNameChanged =
        changes.type === 'legal' &&
        (changes as any).tradeName !== undefined &&
        (changes as any).tradeName !== (client as any).tradeName
      const isTaxIdChanged =
        changes.taxId !== undefined && changes.taxId.value !== client.taxId.value

      if (isNameChanged || isLegalNameChanged || isTradeNameChanged || isTaxIdChanged) {
        throw new CollaboratorNotAuthorizedError(
          'Perfil de Atendimento não tem permissão para alterar CPF/CNPJ ou Nome/Razão Social.',
        )
      }
    }
    ...
    if (changes.taxId && changes.taxId.value !== client.taxId.value) {
      ...
      const existingClient = await this.clientsRepository.findByTaxId({
        ...changes.taxId,
    ...
    const updatedClient = await this.clientsRepository.replace(
      clientId,
      changes,
      auditLogs,
    )
